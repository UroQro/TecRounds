...
export default function PatientFormModal({ onClose, mode, initialData }) {
  ...
  const inputClass = "w-full p-2 border border-slate-300 rounded text-slate-900"; // Keep inputs light for readability
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto text-slate-900">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="bg-slate-50 p-2 rounded border">
                      ...
                      <input placeholder="Alergias" className={`text-xs border-red-200 ${inputClass}`} value={form.allergies} onChange={e=>setForm({...form, allergies:e.target.value})} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                      <div className="flex flex-col"><label className="text-[10px] text-gray-500">Ingreso</label><input type="date" required className={`text-xs ${inputClass}`} value={form.admissionDate} onChange={e=>setForm({...form, admissionDate:e.target.value})} /></div>
                      <div className="flex flex-col"><label className="text-[10px] text-gray-500">Nacimiento</label><input type="date" className={`text-xs ${inputClass}`} value={form.dob} onChange={e=>setForm({...form, dob:e.target.value})} /></div>
                  </div>
