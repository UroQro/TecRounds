...
export default function PatientFormModal({ onClose, mode, initialData }) {
  ...
  // Safe accessor for antecedents to prevent crash
  const getAnt = (key) => form.antecedents ? form.antecedents[key] : false;
  const setAnt = (key, val) => setForm({ ...form, antecedents: { ...form.antecedents, [key]: val } });
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="bg-slate-50 p-2 rounded border">
                      ...
                      <div className="flex gap-2 mb-2 text-sm">
                         <label className="flex items-center gap-1"><input type="checkbox" checked={getAnt('dm')} onChange={e=>setAnt('dm', e.target.checked)}/> DM</label>
                         <label className="flex items-center gap-1"><input type="checkbox" checked={getAnt('has')} onChange={e=>setAnt('has', e.target.checked)}/> HAS</label>
                         <label className="flex items-center gap-1"><input type="checkbox" checked={getAnt('cancer')} onChange={e=>setAnt('cancer', e.target.checked)}/> Onco</label>
                      </div>
                      <input placeholder="Otros antecedentes..." className="w-full p-1 border rounded text-xs mb-2" value={form.antecedents?.other || ''} onChange={e=>setAnt('other', e.target.value)} />
