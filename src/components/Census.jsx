...
export default function Census({ user }) {
  ...
  // VIVID DAY MODE: Use Strong borders + White backgrounds instead of pastel fills
  const getCardColor = (p) => {
    ...
    if (p.type === 'SND') { 
        return p.status === 'done' 
            ? "bg-white dark:bg-green-900/30 border-l-[8px] border-green-600 dark:border-green-500 shadow-md" 
            : "bg-white dark:bg-orange-900/30 border-l-[8px] border-orange-500 dark:border-orange-500 shadow-md"; 
    }
    
    // STANDARD (HO/IC)
    return p.status === 'done' 
        ? "bg-white dark:bg-blue-900/30 border-l-[8px] border-blue-600 dark:border-blue-500 shadow-md" 
        : "bg-white dark:bg-slate-800 border-l-[8px] border-red-600 dark:border-red-500 shadow-md";
  };
            ...
            <div key={p.id} onClick={() => setSelectedPatient(p)} className={`p-3 rounded-lg cursor-pointer active:scale-95 transition ${getCardColor(p)}`}>
               <div className="flex justify-between items-start">
                  <div className="flex-1 pr-2">
                     ...
                     {/* Name is brighter blue in day mode */}
