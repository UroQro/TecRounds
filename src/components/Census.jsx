               ...
               <div className="flex justify-between items-start">
                  <div className="flex-1 pr-2">
                     <div className="flex items-center gap-2 mb-1">
                         ...
                         {p.preDischarge && <span className="text-[10px] font-bold px-2 py-0.5 rounded border shadow-sm uppercase tracking-wider bg-purple-600 text-white">PRE-ALTA</span>}
                     </div>
                     <h3 className="font-bold text-lg text-blue-900 leading-tight">{p.name}</h3>
                     
                     {/* EDAD Y DX EN TARJETA */}
                     <div className="text-sm font-medium text-slate-700 mt-1">{calculateAge(p.dob)} años • {p.diagnosis}</div>

                     <div className="text-xs text-slate-500 flex justify-between bg-white/50 p-1 rounded mt-1"><span>{p.doctor}</span><span className="font-semibold">{p.resident}</span></div>
                  </div>
