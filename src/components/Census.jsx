...
export default function Census({ user }) {
  ...
  // COLORS: Adapt borders/text for dark mode, but keep them vivid in day mode
  const getCardColor = (p) => {
    ...
    if (p.preDischarge) return "bg-purple-50 dark:bg-purple-900/30 border-purple-500 text-slate-900 dark:text-slate-100";
    if (p.hasPending) return "bg-yellow-50 dark:bg-yellow-900/30 border-yellow-500 text-slate-900 dark:text-slate-100";
    if (p.type === 'SND') { return p.status === 'done' ? "bg-green-50 dark:bg-green-900/30 border-green-500 text-slate-900 dark:text-slate-100" : "bg-orange-50 dark:bg-orange-900/30 border-orange-500 text-slate-900 dark:text-slate-100"; }
    return p.status === 'done' ? "bg-blue-50 dark:bg-blue-900/30 border-blue-500 text-slate-900 dark:text-slate-100" : "bg-white dark:bg-slate-800 border-l-8 border-red-500 text-slate-900 dark:text-slate-100";
  };
  ...
  return (
    <div className="pb-24">
      {/* Filter Bar: Stronger shadow and blue border in day mode */}
      ...
      <div className="grid grid-cols-1 gap-3">
         {sortedPatients.filter(p => !filterDoc || p.doctor === filterDoc || (filterDoc === 'Otro' && !DOCTORS.includes(p.doctor))).filter(p => !filterRes || p.resident === filterRes).map(p => (
            /* Cards: Stronger shadow (shadow-md) in day mode for pop */
            <div key={p.id} onClick={() => setSelectedPatient(p)} className={`p-3 rounded-lg shadow-md dark:shadow-sm cursor-pointer active:scale-95 transition ${getCardColor(p)}`}>
               <div className="flex justify-between items-start">
                  <div className="flex-1 pr-2">
                     ...
                     {/* Name: Brighter blue in day mode (blue-700) */}
                     ...
                     <div className="text-xs text-slate-600 dark:text-slate-400 flex justify-between bg-black/5 dark:bg-white/5 p-1 rounded"><span>{p.doctor}</span><span className="font-semibold">{p.resident}</span></div>
                  </div>
                  <div className="flex flex-col items-end justify-between h-full gap-2">
                      {/* Checkbox: Brighter colors */}
                      <button onClick={(e) => toggleStatus(e, p)} className="">{p.status === 'done' ? <CheckSquare size={30} className={p.type === 'SND' ? "text-green-500 dark:text-green-400" : "text-blue-500 dark:text-blue-400"}/> : <Square size={30} className={p.type === 'SND' ? "text-orange-400" : "text-red-400"}/>}</button>
