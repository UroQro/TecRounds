...
export default function Census({ user }) {
  ...
  // COLORS: Maintain identity but adapt borders/text slightly for dark mode visibility
  const getCardColor = (p) => {
    if (p.type === 'NOVER') return "bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-400"; // Darker for ignored
    if (p.preDischarge) return "bg-purple-100 dark:bg-purple-900/30 border-purple-500 text-slate-900"; // Keep colored backgrounds for status visibility
    if (p.hasPending) return "bg-yellow-100 dark:bg-yellow-900/30 border-yellow-500 text-slate-900";
    if (p.type === 'SND') { return p.status === 'done' ? "bg-green-100 dark:bg-green-900/30 border-green-500 text-slate-900" : "bg-orange-100 dark:bg-orange-900/30 border-orange-500 text-slate-900"; }
    return p.status === 'done' ? "bg-blue-100 dark:bg-blue-900/30 border-blue-500 text-slate-900" : "bg-white dark:bg-slate-800 border-l-8 border-red-500 text-slate-900";
  };
            ...
            <div key={p.id} onClick={() => setSelectedPatient(p)} className={`p-3 rounded-lg shadow-sm cursor-pointer active:scale-95 transition ${getCardColor(p)}`}>
               <div className="flex justify-between items-start">
                  <div className="flex-1 pr-2">
                     ...
                     <h3 className="font-bold text-lg text-blue-900 dark:text-blue-300 leading-tight mb-1">{p.name}</h3>
                     <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">{calculateAge(p.dob)} años • {p.diagnosis}</p>
