...
export default function Census({ user }) {
  ...
  const getCardColor = (p) => {
    if (p.type === 'NOVER') return "bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-600 text-gray-400";
    if (p.preDischarge) return "bg-purple-100 dark:bg-purple-900/30 border-purple-500 text-slate-900 dark:text-slate-100";
    if (p.hasPending) return "bg-yellow-100 dark:bg-yellow-900/30 border-yellow-500 text-slate-900 dark:text-slate-100";
    if (p.type === 'SND') { return p.status === 'done' ? "bg-green-100 dark:bg-green-900/30 border-green-500 text-slate-900 dark:text-slate-100" : "bg-orange-100 dark:bg-orange-900/30 border-orange-500 text-slate-900 dark:text-slate-100"; }
    return p.status === 'done' ? "bg-blue-100 dark:bg-blue-900/30 border-blue-500 text-slate-900 dark:text-slate-100" : "bg-white dark:bg-slate-800 border-l-8 border-red-500 text-slate-900 dark:text-slate-100";
  };
  ...
  return (
    <div className="pb-24">
      <div className="bg-white dark:bg-slate-800 p-3 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700 mb-3 sticky top-0 z-10 transition-colors">
            ...
            <div key={p.id} onClick={() => setSelectedPatient(p)} className={`p-3 rounded-lg border-l-8 shadow-sm cursor-pointer active:scale-95 transition ${getCardColor(p)}`}>
               <div className="flex justify-between items-start">
                  <div className="flex-1 pr-2">
                     <div className="flex items-center gap-2 mb-1"><span className="text-xl font-black">{p.bed}</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded border border-black/10 dark:border-white/10 uppercase tracking-wider ${p.type==='NOVER'?'bg-gray-100 dark:bg-slate-700 text-gray-500':'bg-white dark:bg-slate-800 dark:text-white'}`}>{p.type}</span></div>
                     ...
                     <div className="text-xs opacity-75 flex justify-between bg-black/5 dark:bg-white/5 p-1 rounded"><span>{p.doctor}</span><span className="font-semibold">{p.resident}</span></div>
                  </div>
                  <div className="flex flex-col items-end justify-between h-full gap-2">
                      <button onClick={(e) => toggleStatus(e, p)} className="">{p.status === 'done' ? <CheckSquare size={30} className={p.type === 'SND' ? "text-green-600 dark:text-green-400" : "text-blue-600 dark:text-blue-400"}/> : <Square size={30} className={p.type === 'SND' ? "text-orange-400" : "text-red-400"}/>}</button>
                      <div className="flex items-center gap-2 mt-2"><span className="text-[10px] font-bold opacity-60">{calculateLOS(p.admissionDate)}d</span><button onClick={(e) => dischargePatient(e, p)} className="bg-black/10 dark:bg-white/10 p-1 rounded hover:bg-red-100 hover:text-red-500 dark:text-slate-300"><LogOut size={14}/></button></div>
                  </div>
               </div>
            </div>
         ))}
