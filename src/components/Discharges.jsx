...
export default function Discharges({ privacyMode }) {
  ...
  useEffect(() => {
    // Calculamos límite de tiempo para no sobrecargar el celular
    const limitDate = new Date();
    limitDate.setDate(limitDate.getDate() - daysBack);
    const limitStr = limitDate.toISOString();

    // 1. Buscamos en el nuevo archivo histórico (eficiente)
    ...
    // 2. Buscamos los que se quedaron en la tabla vieja por compatibilidad
    const qLegacy = query(collection(db, "patients"), where("discharged", "==", true));
    const unsubLegacy = onSnapshot(qLegacy, (snapshot) => {
      // Filtramos en memoria los legacy para respetar la ventana de tiempo
  ...
  // Juntamos ambos y ordenamos del más reciente al más antiguo
