      ...
      <div className="bg-blue-50 border-b p-3 sticky top-0 z-20 flex gap-2 items-center shadow-sm">
          ...
          <div className="flex-1">
              ...
              <div className="text-xs text-slate-600 mt-1">
                  {patient.bed} • {calculateAge(patient.dob)}a • {patient.doctor}
              </div>
          </div>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded p-3 shadow-sm">
                   ...
                   {patient.checklist?.map((t, i) => (
                       <div key={i} className="flex items-center gap-2 mb-1">
                           <input type="checkbox" checked={t.done} onChange={()=>toggleTask(i)} className="w-5 h-5 accent-yellow-600"/>
                           <span className={`text-sm ${t.done?'line-through text-gray-400':'text-gray-900'}`}>{t.task}</span>
                       </div>
                   ))}
                   <div className="flex gap-2 mt-2">
                       <input className="flex-1 border text-sm p-2 rounded" placeholder="Nuevo pendiente..." value={newTask} onChange={e=>setNewTask(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addTask()}/>
                       <button onClick={addTask} className="bg-yellow-500 text-white px-3 rounded font-bold text-xl">+</button>
                   </div>
                </div>

                <div className="bg-white border rounded-lg p-3 shadow-sm">
                    <div className="flex justify-between mb-2 items-center">
                        <label className="text-xs font-bold text-slate-400 uppercase">Nueva Entrada</label>
                        <select className="text-xs border rounded p-1 bg-slate-50" value={noteType} onChange={e=>setNoteType(e.target.value)}>
                    ...
                    {noteType === 'visita' ? (
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                <input placeholder="TA" className="w-1/3 border text-center text-sm p-2 rounded" value={visitForm.ta} onChange={e=>setVisitForm({...visitForm, ta:e.target.value})}/>
                                <input placeholder="FC" className="w-1/3 border text-center text-sm p-2 rounded" value={visitForm.fc} onChange={e=>setVisitForm({...visitForm, fc:e.target.value})}/>
                                <input placeholder="T°" className="w-1/3 border text-center text-sm p-2 rounded" value={visitForm.temp} onChange={e=>setVisitForm({...visitForm, temp:e.target.value})}/>
                            </div>
                            <div className="flex gap-2">
                                <input placeholder="Gasto U" className="flex-1 border text-center text-sm p-2 rounded" value={visitForm.gu} onChange={e=>setVisitForm({...visitForm, gu:e.target.value})}/>
                                <input placeholder="Drenajes" className="flex-1 border text-center text-sm p-2 rounded" value={visitForm.drains} onChange={e=>setVisitForm({...visitForm, drains:e.target.value})}/>
                            </div>
                            ...
                            <div className="p-2 border rounded bg-slate-50">
                                ...
                                <div className="grid grid-cols-3 gap-1 mb-1">
                                    <input placeholder="Hb" className="lab-input" value={visitForm.hb} onChange={e=>setVisitForm({...visitForm, hb:e.target.value})}/>
                                    <input placeholder="Leu" className="lab-input" value={visitForm.leu} onChange={e=>setVisitForm({...visitForm, leu:e.target.value})}/>
                                    <input placeholder="Plq" className="lab-input" value={visitForm.plq} onChange={e=>setVisitForm({...visitForm, plq:e.target.value})}/>
                                </div>
                                <div className="grid grid-cols-3 gap-1 mb-1">
                                    <input placeholder="Glu" className="lab-input" value={visitForm.glu} onChange={e=>setVisitForm({...visitForm, glu:e.target.value})}/>
                                    <input placeholder="Cr" className="lab-input font-bold bg-yellow-100" value={visitForm.cr} onChange={e=>setVisitForm({...visitForm, cr:e.target.value})}/>
                                    <input placeholder="BUN" className="lab-input" value={visitForm.bun} onChange={e=>setVisitForm({...visitForm, bun:e.target.value})}/>
                                </div>
                                <div className="grid grid-cols-3 gap-1 mb-1">
                                    <input placeholder="Na" className="lab-input" value={visitForm.na} onChange={e=>setVisitForm({...visitForm, na:e.target.value})}/>
                            ...
                            <div className="flex gap-2 pt-2">
                                <button onClick={saveNote} className="flex-1 bg-blue-600 text-white py-3 rounded font-bold text-sm shadow-md">Guardar Nota</button>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            <textarea className="w-full border rounded p-2 text-sm h-20" placeholder={noteType === 'imagen' ? 'Pegar URL de imagen...' : 'Escribir nota...'} value={simpleNote} onChange={e=>setSimpleNote(e.target.value)}/>
                            <button onClick={saveNote} className="w-full bg-blue-600 text-white py-3 rounded font-bold text-sm shadow">Guardar</button>
                        </div>
                    )}
                </div>
                    ...
                    {patient.notes?.slice().reverse().map(note => (
                        <div key={note.id} className="bg-white border rounded p-3 shadow-sm relative group">
                             <div className="flex justify-between items-center text-xs text-gray-400 mb-2 border-b pb-1">
                                 <span>{new Date(note.timestamp).toLocaleDateString('es-MX', {day:'2-digit', month:'short'})} {new Date(note.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} • <span className="text-blue-600 font-bold">{note.author}</span></span>
                                 <div className="flex gap-2 items-center">
                                     <span className="uppercase font-bold bg-slate-100 px-1 rounded text-[10px]">{note.type}</span>
                             ...
                             {note.type === 'visita' ? (
                                 <div className="text-sm space-y-1">
                                     ...
                                     <div className="text-xs bg-slate-50 p-2 rounded border grid grid-cols-2 gap-2 text-slate-600">
                                         <span>TA: {note.content.ta} / FC: {note.content.fc}</span>
                                         <span>T: {note.content.temp} / GU: {note.content.gu}</span>
                                     </div>
                                 ...
                                 <div className="text-sm text-gray-800 break-words">
                                    {note.type === 'imagen' 
                                        ? <a href={note.content.text} target="_blank" className="text-blue-600 underline flex gap-1 items-center"><LinkIcon size={14}/> Ver Imagen</a> 
                                        : note.content.text}
                                 </div>
                             )}
                        </div>
