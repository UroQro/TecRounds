                        ...
                        <div key={note.id} className="bg-white border rounded p-3 shadow-sm relative group">
                             ...
                             {note.type === 'visita' ? (
                                 <div className="text-sm space-y-1">
                                     ...
                                     {/* LABS GRID */}
                                     {(note.content.hb || note.content.cr) && (
                                         <div className="grid grid-cols-3 gap-x-2 gap-y-1 text-[10px] bg-slate-50 p-2 rounded border mt-1 font-mono text-center">
                                             {note.content.hb && <span>Hb: {note.content.hb}</span>}
                                             {note.content.leu && <span>Leu: {note.content.leu}</span>}
                                             {note.content.plq && <span>Plq: {note.content.plq}</span>}
                                             {note.content.glu && <span>Glu: {note.content.glu}</span>}
                                             {note.content.cr && <span className="font-bold bg-yellow-100">Cr: {note.content.cr}</span>}
                                             {note.content.bun && <span>BUN: {note.content.bun}</span>}
                                             {note.content.na && <span>Na: {note.content.na}</span>}
                                             {note.content.k && <span>K: {note.content.k}</span>}
                                             {note.content.cl && <span>Cl: {note.content.cl}</span>}
                                         </div>
                                     )}
                                     <p className="font-medium text-blue-900 mt-1">P: {note.content.plan}</p><button onClick={() => copyMSJ(note.content)} className="mt-2 text-xs bg-green-50 text-green-700 px-2 py-1 rounded border border-green-200 flex items-center gap-1 font-bold w-full justify-center"><Copy size={12}/> Copiar MSJ</button>
                                 </div>
                             ) : note.type === 'sonda' ? (
