...
export default function Surgery({ user }) {
  ...
  const getStyle = (s) => {
      ...
      if (s.completed) { if (s.location === 'Instituto') return "bg-green-100 border-green-500"; return "bg-blue-100 border-blue-600"; }
  ...
  let lastDate = null;
  const filteredList = surgeries.filter(s => { if (s.date < today) return false; if(!filterRes) return true; if(filterRes === 'Por Asignar') return !s.resident; return s.resident === filterRes; });
       ...
       <div className="space-y-3">
           {filteredList.map(s => {
               ...
               <React.Fragment key={s.id}>
                   {showHeader && <h3 className={`text-xs font-bold text-gray-500 uppercase mt-6 mb-2 pl-1 border-b border-gray-200 pb-1 ${s.date > today ? 'opacity-60':''}`}>{dateStr} {s.date === today && <span className="text-blue-600 ml-2">(HOY)</span>}</h3>}
                   <div className={`rounded-lg shadow-sm border-l-8 p-3 relative transition-all duration-300 ${getStyle(s)} ${s.date > today ? 'opacity-60':''}`}>
                       <div className={`flex justify-between text-xs font-bold mb-1 ${s.cancelled ? 'text-gray-400 line-through' : 'text-gray-600'}`}><span>{s.time} hrs</span><span className="bg-white/50 px-2 rounded">{s.location}</span></div>
                       ...
                       <div className={`text-xs mt-1 flex justify-between items-center ${s.cancelled ? 'text-gray-300' : 'text-gray-500'}`}><span>Tx: {s.doctor}</span><span className={`px-2 py-0.5 rounded ${!s.resident && !s.cancelled ? 'bg-yellow-100 text-yellow-700 font-bold' : ''}`}>R: {s.resident || 'NO ASIGNADO'}</span></div>
                       <div className="flex justify-end gap-3 mt-3 border-t border-black/5 pt-2">
                           <button onClick={()=>toggleComplete(s)} className="text-gray-400 hover:text-green-600 transition">{s.completed ? <CheckCircle className="text-green-600 fill-green-100" size={20}/> : <CheckCircle size={20}/>}</button>
                           <button onClick={()=>toggleCancel(s)} className="text-gray-400 hover:text-red-500 transition">{s.cancelled ? <XCircle className="text-red-500" size={20}/> : <XCircle size={20}/>}</button>
      ...
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="space-y-1"><select required={!isOtherDoc} className="w-full p-2 border rounded text-xs" value={isOtherDoc ? 'Otro' : form.doctor} onChange={e=>{ if(e.target.value==='Otro') { setIsOtherDoc(true); setForm({...form, doctor: ''}); } else { setIsOtherDoc(false); setForm({...form, doctor:e.target.value}); }}}> <option value="">Tratante</option>{DOCTORS.map(d=><option key={d} value={d}>{d}</option>)}<option value="Otro">Otro / Agregar...</option></select>{isOtherDoc && <input placeholder="Nombre Tratante" required className="w-full p-2 border border-blue-300 bg-blue-50 rounded text-xs" value={form.doctor} onChange={e=>setForm({...form, doctor:e.target.value})}/>}</div>
                  <div className="space-y-1"><select className="w-full p-2 border rounded text-xs" value={isOtherRes ? 'Otro' : form.resident} onChange={e=>{ if(e.target.value==='Otro') { setIsOtherRes(true); setForm({...form, resident: ''}); } else { setIsOtherRes(false); setForm({...form, resident:e.target.value}); }}}> <option value="">No Asignado</option>{RESIDENTS.map(r=><option key={r} value={r}>{r}</option>)}<option value="Otro">Otro / Agregar...</option></select>{isOtherRes && <input placeholder="Nombre Residente" required className="w-full p-2 border border-blue-300 bg-blue-50 rounded text-xs" value={form.resident} onChange={e=>setForm({...form, resident:e.target.value})}/>}</div>
