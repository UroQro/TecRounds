...
export default function Surgery({ user }) {
  ...
  const getStyle = (s) => {
      if (s.cancelled) return "bg-slate-800 border-slate-700 opacity-50"; // Dark mode cancelled
      if (s.completed) { if (s.location === 'Instituto') return "bg-green-900/50 border-green-500"; return "bg-blue-900/50 border-blue-600"; }
      if (s.location === 'HZH') return "bg-red-900/30 border-red-500";
      if (s.location === 'Instituto') return "bg-orange-900/30 border-orange-500";
  ...
  return (
    <div className="pb-24">
       <div className="flex flex-col gap-2 mb-4 bg-slate-800 p-3 rounded border border-slate-700">
           <div className="flex justify-between items-center"><h2 className="text-xl font-bold text-slate-100 flex items-center gap-2"><Calendar className="text-blue-400"/> Quirófano</h2><button onClick={exportSurgeries} className="bg-green-700 text-white text-xs px-3 py-1 rounded font-bold shadow flex gap-1 items-center hover:bg-green-600"><Download size={14}/> CSV</button></div>
           <select className="border border-slate-600 bg-slate-700 text-white rounded p-1 text-xs w-full" value={filterRes} onChange={e=>setFilterRes(e.target.value)}><option value="">Todos los Residentes</option><option value="Por Asignar">Por Asignar</option>{RESIDENTS.map(r=><option key={r} value={r}>{r}</option>)}</select>
       </div>
       <div className="space-y-3">
           {filteredList.map(s => {
               ...
               <React.Fragment key={s.id}>
                   {showHeader && <h3 className={`text-xs font-bold text-slate-400 uppercase mt-6 mb-2 pl-1 border-b border-slate-700 pb-1 ${opacityClass}`}>{dateStr} {s.date === today && <span className="text-blue-400 ml-2">(HOY)</span>}</h3>}
                   <div className={`rounded-lg shadow-sm border-l-8 p-3 relative transition-all duration-300 ${getStyle(s)} ${opacityClass}`}>
                       <div className={`flex justify-between text-xs font-bold mb-1 ${s.cancelled ? 'text-gray-500 line-through' : 'text-slate-300'}`}><span>{s.time} hrs</span><span className="bg-white/10 px-2 rounded">{s.location}</span></div>
                       <h3 className={`font-bold text-lg leading-tight ${s.cancelled ? 'text-gray-500 line-through' : 'text-white'}`}>{s.patientName}</h3>
                       <p className={`font-medium text-sm mb-2 ${s.cancelled ? 'text-gray-500 line-through' : 'text-blue-300'}`}>{s.procedure}</p>
                       <div className={`text-xs mt-1 flex justify-between items-center ${s.cancelled ? 'text-gray-600' : 'text-gray-400'}`}><span>Tx: {s.doctor}</span><span className={`px-2 py-0.5 rounded text-slate-900 ${!s.resident && !s.cancelled ? 'bg-yellow-400 font-bold' : 'bg-slate-300'}`}>R: {s.resident || 'NO ASIGNADO'}</span></div>
                       <div className="flex justify-end gap-3 mt-3 border-t border-white/10 pt-2">
                           <button onClick={()=>toggleComplete(s)} className="text-gray-500 hover:text-green-400 transition">{s.completed ? <CheckCircle className="text-green-500" size={20}/> : <CheckCircle size={20}/>}</button>
                           <button onClick={()=>toggleCancel(s)} className="text-gray-500 hover:text-red-400 transition">{s.cancelled ? <XCircle className="text-red-500" size={20}/> : <XCircle size={20}/>}</button>
                           <div className="w-px h-5 bg-slate-600 mx-1"></div>
                           <button onClick={()=>handleEdit(s)} className="text-blue-400 hover:text-blue-300"><Edit size={18}/></button>
           ...
           {filteredList.length === 0 && <p className="text-center text-gray-500 mt-10">No hay cirugías próximas.</p>}
       </div>
...
function SurgeryModal({ onClose, initialData }) {
    ...
    // INPUT STYLE FOR DARK MODAL
    const inputClass = "w-full p-2 border border-slate-300 rounded text-slate-900"; 

    return (
      <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto text-slate-900">
