...
function SurgeryModal({ onClose, initialData }) {
    ...
    // Safety check for old records without resident2
    useEffect(() => {
      ...
      <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto text-slate-900 dark:text-white">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  {/* RESIDENT 1 SELECT */}
                  <div className="space-y-1"><label className="text-xs font-bold text-gray-500">Residente Principal</label><select className={`text-xs ${inputClass}`} value={isOtherRes ? 'Otro' : form.resident} onChange={e=>{ if(e.target.value==='Otro') { setIsOtherRes(true); setForm({...form, resident: ''}); } else { setIsOtherRes(false); setForm({...form, resident:e.target.value}); }}}> <option value="">No Asignado</option>{RESIDENTS.map(r=><option key={r} value={r}>{r}</option>)}<option value="Otro">Otro / Agregar...</option></select>{isOtherRes && <input placeholder="Nombre Residente" required className={`text-xs bg-blue-50 ${inputClass}`} value={form.resident} onChange={e=>setForm({...form, resident:e.target.value})}/>}</div>
                  
                  {/* RESIDENT 2 SELECT */}
                  <div className="space-y-1"><label className="text-xs font-bold text-gray-500">2do Residente (Opcional)</label><select className={`text-xs ${inputClass}`} value={form.resident2 || ''} onChange={e=>setForm({...form, resident2:e.target.value})}> <option value="">Ninguno</option>{RESIDENTS.map(r=><option key={r} value={r}>{r}</option>)}</select></div>
